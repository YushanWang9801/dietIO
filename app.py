...
def home_page():
    ...
    st.write("Let us start to build your fugure and keep monitoring your health.")

    cover_image = Image.open("./cover.jpg")
    st.image(cover_image, caption="Cover image of DIET.IO", use_column_width=True)
    st.write("First, please click the button below to enter the data entry page to log in your personal information. ")

    if st.button("Go to your Data Entry page: "):
...
def entry_page():
    ...
    with col1:
        st.write("Please enter basic Health Information. ")
        name = st.text_input("Please enter your name:  (format LastName_FirstName)")
        age = st.text_input("Please enter your age: (Integer Please)")
        height = st.text_input("Please enter your height: (Nearest Integer in centimeters)")
        weight = st.text_input("Please enter your weight: (Nearest Integer in kg)")
    # Add text input boxes to the second column
    with col2:
        st.write("Now please enter a few personal preferences: ")
        place = st.text_input("Please enter one of your favorite place: (Please one of the location on earth) ")
        breakfast_time = st.time_input("Enter a time that you normally have breakfast", value=None)
        lunch_time = st.time_input("Enter a time that you normally have lunch", value=None)
        dinner_time = st.time_input("Enter a time that you normally have dinner", value=None)
    ...
    selfie_image = None
    if name:
        st.write(f"Hello, {name}!, welcome to this website")
        if age:
            try:
                age = int(age)
            except ValueError:
        ...
        if place:
            st.write(f"{place}, Nice choice! That is a beautiful place.")
        ...
        if age and height and weight and place \
            ...
            st.title("Upload your selfie image: ")
